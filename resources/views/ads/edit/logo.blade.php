...
@section('content')
    ...
    <div class="report-wrapper">
        <h5 class="page-title d-flex">
                    <span>
                    <i class="fas fa-bullhorn"></i> Ads Logo
                    </span>
        </h5>

        <div class="ads-banner-wrapper">
            <form id="submitForm" action="{{route('ads.update', [$ad])}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="row">
                    <div class="col-xl-6">
                        ...
                        <div class="form-group">
                            ...
                            <select name="time_of_appearance" id="">
                                ...
                                @for($i = 3; $i <= 15; $i++)
                                    <option
                                        value="{{ $i }}" {{ (old('time_of_appearance') ?: $ad->time_of_appearance) == $i ? ' selected' : null }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="selectbox no-bg">
                                <select name="link_type" id="">
                                    <option value="">Select any</option>
                                    <option value="external" {{ (old('link_type') ?: $ad->link_type) == 'external' ? ' selected' : null }}>
                                        External
                                    </option>
                                    <option value="internal" {{ (old('link_type') ?: $ad->link_type) == 'internal' ? ' selected' : null }}>
                                        Internal
                                    </option>
                                </select>
                        ...
                        <div class="form-group adLink">
                            ...
                            <input type="text" name="link_external" class="form-control-sm form-control"
                                   value="{{ old('link_external') ?: $ad->link }}"/>
                            <select name="link_internal">
                                <option value="">Select any</option>
                                @foreach((new \App\Services\AdService())->getAppInternalPages() as $key => $value)
                                    <option value="{{ $key }}" @selected((old('link_internal') ?: $ad->link) == $key)>{{ $value }}</option>
                                @endforeach
                            </select>
                                ...
                                <select name="country" id="country">
                                    ...
                                    @isset($countries)
                                        @foreach($countries as $country)
                                            <option
                                                value="{{$country->id}}" {{ (old('country') ?: $ad->country->id) == $country->id ? ' selected' : null }}>{{$country->name}}</option>
                                        @endforeach
                                    @endisset
                                </select>
                                @include('shared.errors.validation', ['name' => 'country'])
                            </div>
                        </div>
                                ...
                                <select name="league" id="league">
                                    ...
                                    @isset($leagues)
                                        @foreach($leagues as $league)
                                            <option
                                                value="{{$league->id}}" {{ (old('league') ?: $ad->league_id) == $league->id ? ' selected' : null }}>{{$league->name}}</option>
                                        @endforeach
                                    @endisset
                                </select>
                                @include('shared.errors.validation', ['name' => 'league'])
                            </div>
                        </div>
                        ...
                        <div class="form-group date-group">
                            ...
                            <div class="from">
                                ...
                                <input type="date" name="started_at"
                                       value="{{ old('started_at') ?: $ad->started_at }}"/>
                            </div>
                            @include('shared.errors.validation', ['name' => 'started_at'])
                            <div class="to">
                                <span>to</span>
                                <input type="date" name="ended_at" value="{{ old('ended_at') ?: $ad->ended_at }}"/>
                            </div>
